...
function UI() {
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          if (entry.target.classList.contains('ui-card')) {
            anime({
              targets: entry.target,
              translateY: [50, 0],
              opacity: [0, 1],
    ...
    document.querySelectorAll('.section-title, .ui-card').forEach(el => {
      observer.observe(el)
    })
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#FF7F6E] to-[#FFA07A] py-10 sm:py-20 px-4 text-white">
      <div className="container mx-auto max-w-7xl">
        ...
        <p className="text-white/90 text-lg sm:text-xl text-center mb-10 sm:mb-16 max-w-3xl mx-auto leading-relaxed">
          Explore a collection of meticulously crafted UI projects that seamlessly blend aesthetics with functionality, 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {uiProjects.length === 0 ? (
            <div className="col-span-full text-center py-10 sm:py-20 bg-white/10 backdrop-blur-lg rounded-3xl">
              <h2 className="text-2xl sm:text-3xl font-semibold text-white mb-4">No UI Projects Available</h2>
              <p className="text-white/80 text-base sm:text-lg">Check back soon for exciting new UI designs!</p>
            </div>
          ) : (
            uiProjects.map((ui, index) => (
              <div
                key={index}
                className="ui-card group relative bg-white/10 backdrop-blur-lg rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 opacity-0 flex flex-col"
              >
                ...
                <div className="p-4 sm:p-6 flex flex-col flex-grow relative z-10">
                  ...
                  <p className="text-white/80 text-sm mb-4 leading-relaxed">{ui.desc}</p>
